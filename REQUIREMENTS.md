...
- Such clients register via **Dynamic Client Registration (RFC 7591)** or **CIMD** (Client ID
  Metadata Document) — so the server must offer DCR (and/or CIMD).
...
- **FR-2 Dynamic Client Registration:** `POST /register` (RFC 7591). Optionally also accept
  **CIMD** client IDs (config flag) to avoid persisting registrations.
...
## 6. Open decisions (tracked in PROGRESS.md)
- Build vs fork an existing project (F-001).
- Implementation language + OAuth library (Go + Ory Fosite vs Python + authlib).
- CIMD support vs DCR-only.
- Persistence choice (e.g. SQLite/file) and multi-user evolution.
